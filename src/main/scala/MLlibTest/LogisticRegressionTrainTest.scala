...
object LogisticRegressionTrainTest {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().master("local[2]").appName("LinearRegression").getOrCreate()

//    加载数据
    val data = spark.read
      .option("header","false")
      .option("inferSchema","true")
      .csv("./src/main/resources/MLlibData/tmp_matrix_train_l5brPwufXccPBww.log")
    ...
    val chiSqData = chiSq.fit(vecDF).transform(vecDF)

    val lrModel = new LogisticRegression().setLabelCol("crime_bi").setFeaturesCol("selectFeatures").fit(chiSqData)


//    val lrSummary = lrModel.summary
    println(s"Coefficients: ${lrModel.coefficients} Intercept: ${lrModel.intercept}")
    lrModel.save("./src/main/resources/MLlibData/model/logisticMode1")
    spark.stop()
  }
